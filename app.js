  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = () => {
      ...
      img.onload = () => {
        ...
        // JPEG at moderate quality keeps the embedded size well within the
        // spreadsheet cell's character limit even for a larger image.
...
function wireAdminPanel() {
  ...
  document.getElementById('addAppBtn').onclick = async () => {
    ...
    try {
      ...
      await apiPost('addApp', { name, url, icon, description, thumbnail: pendingThumbDataUrl || '' });
  ...
  if (saveEditBtn) {
    saveEditBtn.onclick = async () => {
      ...
      try {
        ...
        await apiPost('updateApp', params);
        state.editingAppId = null;
        pendingEditThumbDataUrl = null;
        toast('App updated');
        await refreshApps();
