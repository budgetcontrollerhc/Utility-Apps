...
function renderMain() {
  appEl.innerHTML = `
    ...
    <main>
      ...
      <div class="grid" id="grid" style="--tile-scale:${state.settings.tileTextScale};"></div>
...
function renderAdminPanel() {
  ...
  return `
    <div class="admin-section">
      <h2>Display</h2>
      <p class="section-sub">Adjust the name/description text size on app tiles. Drag to preview live, then Save.</p>
      <div class="inline-form" style="align-items:center;">
        ...
        <button class="btn primary small" id="saveTileScaleBtn">Save</button>
      </div>
    </div>
    ${pending.length ? `
    <div class="admin-section">
...
function wireAdminPanel() {
  ...
  if (scaleSlider) {
    ...
    document.getElementById('saveTileScaleBtn').onclick = async () => {
      const val = Number(scaleSlider.value);
      const data = await apiGet('updateSettings', { tileTextScale: val });
      if (data.error) { toast('Could not save: ' + data.error); return; }
      state.settings.tileTextScale = val;
      toast('Text size saved for everyone');
    };
  }
