...
let state = {
  ...
  editingAppId: null,
  settings: { tileTextScale: 1, tileLayout: 'comfortable', bgImage: '' }
};
...
async function checkSession() {
  try {
    ...
    state.apps = data.apps || []; if (data.settings) state.settings = data.settings;
    renderMain();
  } catch (err) {
...
function signOut() {
  apiGet('logout').catch(() => {});
  localStorage.removeItem('launcher_token');
  document.body.dataset.hasBg = 'false';
  document.body.style.setProperty('--custom-bg', '');
  state = { token: null, mobile: null, name: null, isAdmin: false, apps: [], users: [], adminOpen: false, authMode: 'login', editingAppId: null, settings: { tileTextScale: 1, tileLayout: 'comfortable', bgImage: '' } };
  renderAuth();
}

const LAYOUT_WIDTHS = { compact: '140px', comfortable: '190px', large: '260px' };

function applyTheming() {
  document.body.dataset.hasBg = state.settings.bgImage ? 'true' : 'false';
  document.body.style.setProperty('--custom-bg', state.settings.bgImage ? `url("${state.settings.bgImage}")` : '');
}
...
function renderMain() {
  ...
  appEl.innerHTML = `
    ...
    <main>
      ...
      <div class="grid" id="grid" style="--tile-scale:${state.settings.tileTextScale}; --tile-min:${LAYOUT_WIDTHS[state.settings.tileLayout] || LAYOUT_WIDTHS.comfortable};"></div>
...
function renderGrid() {
  ...
  grid.innerHTML = filtered.map((a, i) => `
    <a class="tile" style="animation-delay:${Math.min(i * 40, 400)}ms" href="${escapeAttr(withAuthUser0(a.url))}" target="_blank" rel="noopener noreferrer">
      ${a.thumbnail
...
function renderAdminPanel() {
  ...
  return `
    <div class="admin-section">
      <h2>Display</h2>
      <p class="section-sub">Preview live, then Save applies it for everyone.</p>

      <div style="margin-bottom:14px;">
        <label style="display:block; font-size:12.5px; color:var(--sub); margin-bottom:6px;">Text size</label>
        <div class="inline-form" style="align-items:center;">
          <input type="range" id="tileScaleSlider" min="0.7" max="1.4" step="0.05" value="${state.settings.tileTextScale}" style="flex:1;">
          <span class="chip" id="tileScaleValue">${Math.round(state.settings.tileTextScale * 100)}%</span>
        </div>
      </div>

      <div style="margin-bottom:14px;">
        <label style="display:block; font-size:12.5px; color:var(--sub); margin-bottom:6px;">Tile size / layout</label>
        <div class="inline-form" id="layoutButtons">
          ${['compact', 'comfortable', 'large'].map(l => `
            <button type="button" class="btn small${state.settings.tileLayout === l ? ' primary' : ''}" data-layout="${l}">${l[0].toUpperCase() + l.slice(1)}</button>
          `).join('')}
        </div>
      </div>

      <div style="margin-bottom:14px;">
        <label style="display:block; font-size:12.5px; color:var(--sub); margin-bottom:6px;">Background image (optional)</label>
        <div class="thumb-row">
          <div class="thumb-preview" id="bgPreview">${state.settings.bgImage ? `<img src="${state.settings.bgImage}" alt="">` : 'None'}</div>
          <label class="thumb-file-label" for="bgFile">Choose image</label>
          <input type="file" id="bgFile" accept="image/*" style="display:none;">
          ${state.settings.bgImage ? '<button type="button" class="btn small danger" id="removeBgBtn">Remove</button>' : ''}
        </div>
      </div>

      <div class="form-actions">
        <button class="btn primary" id="saveDisplayBtn">Save display settings</button>
      </div>
    </div>
    ${pending.length ? `
    <div class="admin-section">
...
function wireAdminPanel() {
  let pendingBgDataUrl = undefined; // undefined = no change; '' = explicit removal
  let pendingLayout = state.settings.tileLayout;

  const scaleSlider = document.getElementById('tileScaleSlider');
  if (scaleSlider) {
    scaleSlider.oninput = () => {
      const val = Number(scaleSlider.value);
      document.getElementById('tileScaleValue').textContent = Math.round(val * 100) + '%';
      document.getElementById('grid').style.setProperty('--tile-scale', val);
    };
  }

  document.querySelectorAll('[data-layout]').forEach(btn => {
    btn.onclick = () => {
      pendingLayout = btn.getAttribute('data-layout');
      document.getElementById('grid').style.setProperty('--tile-min', LAYOUT_WIDTHS[pendingLayout]);
      document.querySelectorAll('[data-layout]').forEach(b => b.classList.toggle('primary', b === btn));
    };
  });
  ...
  if (bgFileInput) {
    bgFileInput.onchange = async () => {
      ...
      if (!file) return;
      const dataUrl = await resizeImageToDataUrl(file, 900, 0.5);
      if (dataUrl.length > 45000) { toast('Background image is too large even after compressing — try a simpler image'); return; }
      pendingBgDataUrl = dataUrl;
      document.getElementById('bgPreview').innerHTML = `<img src="${dataUrl}" alt="">`;
      document.body.dataset.hasBg = 'true';
  ...
  if (removeBgBtn) {
    removeBgBtn.onclick = () => {
      pendingBgDataUrl = '';
      document.getElementById('bgPreview').innerHTML = 'None';
      document.body.dataset.hasBg = 'false';
      document.body.style.setProperty('--custom-bg', '');
    };
  }
  ...
  document.getElementById('saveDisplayBtn').onclick = async () => {
    ...
    const params = { tileTextScale: Number(scaleSlider.value), tileLayout: pendingLayout };
    if (pendingBgDataUrl !== undefined) params.bgImage = pendingBgDataUrl;
    try {
      const data = await apiPost('updateSettings', params);
      if (data.error) { toast('Could not save: ' + data.error); btn.textContent = 'Save display settings'; btn.disabled = false; return; }
      state.settings = data.settings;
      toast('Display settings saved for everyone');
      renderMain();
    } catch (err) {
      toast('Could not save display settings');
      btn.textContent = 'Save display settings'; btn.disabled = false;
    }
  };
...
function withAuthUser0(url) {
  if (!url.includes('google.com')) return url; // only relevant for Google-hosted links
  return url + (url.includes('?') ? '&' : '?') + 'authuser=0';
}
